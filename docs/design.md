...
**起動時のプロファイル選択**
- 履歴に前回のプロファイルがあれば自動的に開き、StartupPage を経由せず直接 IndexPage / ViewerPage に遷移
- 自動オープン失敗時（ファイル削除・移動等）はエラーなしで StartupPage を表示
- 履歴がない場合（初回起動）は StartupPage を表示してファイルダイアログまたは新規作成を促す

**エラー状態のカード**
- 参照先フォルダが存在しない場合、カードをエラー状態として表示
- エラー状態のカードは編集（パス変更）・削除のみ可能
- ビューアで開くことは不可
- サムネイル元画像が削除・移動された場合は、エラー表示なし・画像なしアイコンをサイレントに表示（カードは通常通り開ける）
...
```
              ...
              ┌─────────────────────┐
              │ app_config 読み込み  │
              │ 前回プロファイル先読 │ → InitialState として管理状態に保持
              └─────────────────────┘
                       │
                       ▼ getInitialState() 1コマンド（ファイルIOなし）
              ┌─────────────────┐
              │  プロファイルあり？ │
              └─────────────────┘
               YES │        NO │
...
エラーメッセージは表示されず、カードに画像なしアイコンが表示される。カードは通常通り開くことができる。再設定するにはカード編集からサムネイルを再選択する。
...
### Phase 2: プロファイル管理 ✅ 完了
- [x] 起動時のプロファイル選択フロー（StartupPage）
- [x] 最近使用したプロファイル履歴
- [x] 新規プロファイル作成
- [x] プロファイル切り替え（IndexPageヘッダー）
- [x] 別名保存
- [x] 起動時の前回プロファイル自動オープン（履歴あり時は StartupPage をスキップ）
...
| 項目 | 現状 | 対応内容 | 対象ファイル |
|------|------|---------|------------|
| ディスクサムネイルキャッシュ ✅ | `app_cache_dir()/thumbnails/`にJPEG保存済み | SHA-256ハッシュをファイル名に使用。元画像のmtimeでキャッシュ有効性を判定。メモリ→ディスク→生成の3層キャッシュ | `images.rs`, `Cargo.toml`（sha2追加） |
| 同時リクエスト制御（低優先） | 制御なし（カード数分同時発行） | 同時実行数を制限するキュー（4並列程度） | 新規 `thumbnailCache.ts` |
| 大量カード時の仮想スクロール | 全カード同時レンダリング | React Window等で可視範囲のみレンダリング | `CardGrid.tsx` |
| フロントエンドサムネイルキャッシュ（低優先） | なし（Rust側メモリ+ディスクキャッシュで十分高速） | ディスクキャッシュ導入によりIPC往復（~0.1ms）の削減のみが効果。カード200枚超でなければ不要 | 新規 `thumbnailCache.ts`, `CardItem.tsx` |
...
| 起動フリッカー解消 ✅ | ウィンドウ表示時に描画前の画面が一瞬見える | `visible: false` で非表示起動。Rust `setup` フックで `InitialState` を事前読み込み。フロントエンドが `getInitialState()` 1コマンドで即時取得し、ウィンドウサイズ・位置復元後に `window.show()` | `lib.rs`, `models/initial_state.rs`, `commands/initial_state.rs`, `profileStore.ts`, `IndexPage.tsx`, `StartupPage.tsx`, `tauri.conf.json`, `default.json` |
...
- [ ] 履歴の最大保持数（デフォルト10件）
- [ ] **アプリ識別子（identifier）の変更**: 現在 `org.example.image-folder-viewer`（`src-tauri/tauri.conf.json`）を使用中。リリース前に正式なドメインに変更が必要。変更時は `app_data_dir()` / `app_cache_dir()` のパスが変わるため、既存データの移行を考慮すること
...
#### 改善4: ディスクキャッシュ（優先度: 低、難易度: 高）✅ 実装済み

`app_cache_dir()/thumbnails/` 配下にJPEGファイルとして保存し、アプリ再起動後も高速に読み込む。
...
- ファイル名は `SHA-256("{image_path}:{size}")` のハッシュ値（`sha2`クレート使用）
- 元画像のmtimeとキャッシュファイルのmtimeを比較してキャッシュ有効性を判定
- `get_thumbnail` に `AppHandle` 引数を追加（Tauri v2が自動注入、フロントエンド変更不要）
- ディスクI/Oエラー時はサイレントにフォールバック（メモリキャッシュ+毎回生成で動作）
- キャッシュフロー: メモリキャッシュ → ディスクキャッシュ → サムネイル生成 の3層構造
- ディスクヒット時はメモリキャッシュにも格納し、2回目以降のアクセスでファイルI/O不要
...
| 3 | フロントエンドキャッシュ | 再レンダリング時の再取得防止 | 中 | 低優先（ディスクキャッシュで十分） |
| 4 | ディスクキャッシュ | アプリ再起動後も高速 | 中 | ✅ 実装済み |
